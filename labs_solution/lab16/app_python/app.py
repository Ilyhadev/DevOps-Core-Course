"""
DevOps Info Service - Flask with visit counter persisted to /data/visits (Lab 12)
"""
...
SERVICE_DESCRIPTION = "DevOps course info service (with persistence)"
FRAMEWORK = "Flask"
...
def ensure_data_dir():
    try:
        ...
        logger.exception("Failed to create data directory", extra={"path": DATA_DIR})
...
def read_visits():
    ...
    try:
        ...
        logger.exception("Failed to read visits file")
        return 0
...
def write_visits(count):
    ...
    try:
        ...
        logger.exception("Failed to write visits file")
...
def index():
    ...
    # Increment visits counter (protected by a simple lock)
    try:
        with _file_lock:
            current = read_visits()
            current += 1
        ...
        logger.exception("Error updating visits count")
...
def visits():
    try:
        ...
        logger.exception("Failed reading visits for /visits")
        current = 0
    endpoint_calls.labels(endpoint="/visits").inc()
    return jsonify({"visits": current})
